...
>
> **Use MCP first.** When the PR0TA MCP connector is available, read/save full sequences with `post_sequence_get` and `post_sequence_save`, start renders with `post_render_start`, and use REST timeline routes only for fine-grained clip/track/editorial primitives that are not exposed through MCP yet.
>
