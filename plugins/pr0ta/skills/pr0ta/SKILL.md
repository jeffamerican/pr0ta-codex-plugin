...
| Audio generation (ElevenLabs TTS) | `POST /generate` | `generator=audio`, `mode=txt_to_speech` |
...
7. **Narration** — Generate narration for segments where the speaker is **not** visible. English dialogue uses native video audio (step 5). Non-English dialogue uses ElevenLabs TTS regardless. See `pr0ta-audio`.
...
For the complete API-first pipeline (auth, per-endpoint requests, reliability contract, download fallbacks, reference uploads, and UI navigation), read `pr0ta-api` and `pr0ta-downloading`. They carry the schemas and the state machine; this hub carries only the orchestration.
...
- **Generating narration or dialogue?** Read `pr0ta-audio`. ElevenLabs v3 TTS, Gemini Flash TTS, voice discovery, voice cloning, STS, English vs non-English dialogue handling, Scribe V2 transcription.
...
- **Sending assets for client review?** Read `pr0ta-api` → "Client Review Room API". Enable Studio mode, submit assets, share review URLs, retrieve timestamped annotations and approval decisions, completion webhook.
